...
def convert_train_valid_to_dataset(iset,istart,target_train,target_valid,features_train1,features_valid1,feature_varnames1,features_train2,features_valid2,feature_varnames2,features_train3,features_valid3,feature_varnames3,start_doy,ws,ns):
    ...
    for ivar in range(features_train1.shape[0]):
        for iw in range(features_train1.shape[3]):
            ...
            # trainset[:,v] =  target_train[:,iset] + np.random.normal(0,7,len(target_train[:,iset]))
            # np.random.seed(seed=23)
            # validset[:,v] =  target_valid[:,iset] + np.random.normal(0,7,len(target_valid[:,iset]))
            # pred_list += [monthly_str[iw] +' '+ 'TEST FUD' + str(ivar)]
    ...
    for ivar in range(features_train2.shape[0]):
        for iw in range(features_train2.shape[3]):
            ...
            # np.random.seed(seed=23)
            # validset[:,v] =  target_valid[:,iset] + np.random.normal(0,7,len(target_valid[:,iset]))
            # pred_list += [threemonthly_str[iw] +' '+ 'TEST FUD' + str(ivar)]
            v+=1
...
def stepwiseSelection(X, y, columns,
    ...
    while True:
        ...
        if use_Radj:
            new_Radj = pd.Series(index=remaining_features, dtype='float64')
        if use_fpval:
            f_pval = pd.Series(index=remaining_features, dtype='float64')
        for new_column in remaining_features:
            ...
            if use_Radj:
                new_Radj[new_column] = model.rsquared_adj
            if use_fpval:
                f_pval[new_column] = model.f_pvalue
        min_p_value = new_pval.min()
        if use_Radj:
            min_Radj = new_Radj[np.argmin(new_pval)]
            ...
            min_fpval = f_pval[np.argmin(new_pval)]
            if (min_p_value < threshold_in) & (min_fpval < threshold_in):
                best_feature = new_pval.idxmin()
                included.append(best_feature)
                changed=True
                if verbose:
        ...
        if use_fpval:
            if (worst_pval > threshold_out) | (model.f_pvalue >  threshold_out):
                changed=True
...
#         369., 349., 370., 384., 374., 358., 376., 354.])[12:]
...
discharge_monthly = monthly_vars_tmp[19,:,:]
NAO_monthly = monthly_vars_tmp[16,:,:]
Tw_monthly = monthly_vars_tmp[15,:,:]
FDD_monthly = monthly_vars_tmp[10,:,:]
clouds_monthly = monthly_vars_tmp[7,:,:]
precip_monthly = monthly_vars_tmp[6,:,:]
snow_monthly = monthly_vars_tmp[5,:,:]
Ta_mean_monthly = monthly_vars_tmp[0,:,:]
Ta_min_monthly = monthly_vars_tmp[1,:,:]
SW_monthly = monthly_vars_tmp[12,:,:]
LH_monthly = monthly_vars_tmp[13,:,:]
SH_monthly = monthly_vars_tmp[14,:,:]

# fig,ax = plt.subplots()
# ax.plot(discharge_monthly[:,9],avg_freezeup_doy,'o')
# ax.set_xlabel('October mean discharge')
# ax.set_ylabel('FUD DOY')

# fig,ax = plt.subplots()
# ax.plot(discharge_monthly[:,10],avg_freezeup_doy,'o')
# ax.set_xlabel('Nov. mean discharge')
# ax.set_ylabel('FUD DOY')

# fig,ax = plt.subplots()
# ax.plot(NAO_monthly[:,8],avg_freezeup_doy,'o')
# ax.set_xlabel('Sept. mean NAO')
# ax.set_ylabel('FUD DOY')

# fig,ax = plt.subplots()
# ax.plot(snow_monthly[:,10],avg_freezeup_doy,'o')
# ax.set_xlabel('Nov. tot. snow')
# ax.set_ylabel('FUD DOY')

# fig,ax = plt.subplots()
# ax.plot(precip_monthly[:,10],avg_freezeup_doy,'o')
# ax.set_xlabel('Nov. tot. precip')
# ax.set_ylabel('FUD DOY')
...
fig,ax = plt.subplots()
x=Tw_monthly[:,11]
y = Ta_mean_monthly[:,11]
ax.plot(x,y,'o')
ax.set_xlabel('Dec. mean Tw')
ax.set_ylabel('Dec. Ta_mean')
m = sm.OLS(y,sm.add_constant(x),missing='drop').fit()
print(m.rsquared,m.f_pvalue)
#%%
fig,ax = plt.subplots()
ax.plot(Ta_mean_monthly[:,10],avg_freezeup_doy,'o')
ax.set_xlabel('Nov mean Ta_mean.')
ax.set_ylabel('FUD DOY')
m = sm.OLS(avg_freezeup_doy,sm.add_constant(Ta_mean_monthly[:,10]),missing='drop').fit()
print(m.rsquared,m.f_pvalue)
#%%
fig,ax = plt.subplots()
x = Ta_mean_monthly[:,9]
ax.plot(x,avg_freezeup_doy,'o')
ax.set_xlabel('Oct mean Ta_mean.')
ax.set_ylabel('FUD DOY')
m = sm.OLS(avg_freezeup_doy,sm.add_constant(x),missing='drop').fit()
print(m.rsquared,m.f_pvalue)

fig,ax = plt.subplots()
x = Ta_mean_monthly[:,11]
ax.plot(x,avg_freezeup_doy,'o')
ax.set_xlabel('Dec mean Ta_mean.')
ax.set_ylabel('FUD DOY')
m = sm.OLS(avg_freezeup_doy,sm.add_constant(x),missing='drop').fit()
print(m.rsquared,m.f_pvalue)
...
y= Tw_monthly[:,3]
ax.plot(x,y,'o')
ax.set_xlabel('April Latent Heat')
ax.set_ylabel('April Tw')
m = sm.OLS(y,sm.add_constant(x),missing='drop').fit()
print(m.rsquared,m.f_pvalue)#%%
#%%
fig,ax = plt.subplots()
...
# np.savez('MLR_all_predictors',
#         years = years,
#         avg_freezeup_doy = avg_freezeup_doy,
#         monthly_vars= monthly_vars,
#         varnames_monthly = varnames_monthly,
#         threemonthly_vars = threemonthly_vars,
#         varnames_threemonthly = varnames_threemonthly,
#         window_vars = window_vars,
#         varnames_window = varnames_window
# )
    ...
    for valid_opt in range(3):
        ...
        for istart in range(5,6):
            ...
            for iset in range(nsets):
                ...
                # rf = []
                # for i in range(1,len(cor_target)):
                #     if np.all(~np.isnan(threshold[i])):
                #         if cor_target[i]>=threshold[i][0]:
                #             rf.append(cor_target.index[i])

                # relevant_features = cor_target[rf]
                ...
                cfb = stepwiseSelection(x,Y,df_clean.columns[1:].values,verbose=False,threshold_in=0.01,threshold_out = 0.05)
                # cfb = stepwiseSelection(x,Y,df_clean.columns[1:].values,verbose=False,threshold_in=0.05,threshold_out = 0.1)
                # if ntraining == 18:
                print('')
                print('====================================')
                print('Set ' + str(iset))
...
# np.random.seed(seed=23)
# plt.figure()
# plt.plot(years,avg_freezeup_doy,'o-',color='black')

# np.random.seed(seed=23)
# t1 =  np.squeeze(FUD_train_obs) + np.random.normal(0,7,len(np.squeeze(FUD_train_obs)))
# np.random.seed(seed=23)
# t2 =  np.squeeze(FUD_valid_obs) + np.random.normal(0,7,len(np.squeeze(FUD_valid_obs)))

# plt.plot(np.concatenate((np.squeeze(years_train),np.squeeze(years_valid))),np.concatenate((t1,t2)),'o-',color=plt.get_cmap('tab20')(0))
