...
def get_train_valid_test_metric_dfs(valid_scheme,pred_df_clean,avg_freezeup_doy,all_combinations,nsplits,nfolds,years,train_yr_start,valid_yr_start,test_yr_start,tercile1_FUD,tercile2_FUD,p_critical,max_pred):
    ...
    if valid_scheme == 'LOOk':
        ...
        targets = avg_freezeup_doy[0:it_2019+1]
        ...
        print(clim_test)
        plt.figure()
        plt.plot(years, targets)
        plt.plot(years,clim_test,'*-')
...
avg_freezeup_doy = avg_freezeup_doy[it_1992:it_2020+1]
...
for ind,istart in enumerate(istart_show):
    ...
    pred_df_clean, dropped_cols = remove_collinear_features(pred_df, avg_freezeup_doy, threshold=0.8, target_in_df = False)
    ...
    # PLOT TOP 5 MODELS FOR RMSE AND MAE
    if ~np.all(np.isnan(pd.to_numeric(df_valid_all['valid_RMSE']))):
        ...
        df_subset = df_valid_MAE.nsmallest(5, 'valid_MAE')
        for nmodel in range(np.min([len(df_subset),5])):
        ...
        axMAE.legend()
        plt.title('Top 5 MLR models according to '+ 'MAE' + ' - ' + valid_scheme + ' valid. ('+ istart_labels[istart] +')')
        ...
        df_subset = df_valid_RMSE.nsmallest(5, 'valid_RMSE')
        for nmodel in range(np.min([len(df_subset),5])):
            nmodel = df_subset.index.values[nmodel]
            plot_label = df_test_all.iloc[nmodel]['predictors']
            axRMSE.plot(test_years,df_test_all.iloc[nmodel]['test_predictions'],'o-',label=plot_label)
        axRMSE.legend()
        plt.title('Top 5 MLR models according to '+ 'RMSE' + ' - ' + valid_scheme + ' valid. ('+ istart_labels[istart] +')')
    ...
    if (valid_scheme == 'LOOk') | (valid_scheme == 'standardk'):
        if ~np.all(np.isnan(pd.to_numeric(df_valid_all['valid_RMSE']))):
            ...
            for imodel in range(np.min([len(df_subset),5])):
                nmodel = df_subset.index.values[imodel]
                plot_label = df_test_all.iloc[nmodel]['predictors']
                if valid_scheme == 'LOOk':
                    for iyr in range(len(test_years)):
                        axCV_MAE.plot((imodel+32*imodel)+(iyr*1),df_valid_all.iloc[nmodel]['valid_MAE_mean'][iyr],'x',color=plt.get_cmap('tab20')(imodel*2))
            ...
            for imodel in range(np.min([len(df_subset),5])):
                ...
                if valid_scheme == 'LOOk':
                    for iyr in range(len(test_years)):
                        axCV_RMSE.plot((imodel+32*imodel)+(iyr*1),df_valid_all.iloc[nmodel]['valid_RMSE_mean'][iyr],'x',color=plt.get_cmap('tab20')(imodel*2))
...
plt.title('Best MLR models according to '+valid_metric + ' - ' + valid_scheme + ' valid.')
ax.legend()
