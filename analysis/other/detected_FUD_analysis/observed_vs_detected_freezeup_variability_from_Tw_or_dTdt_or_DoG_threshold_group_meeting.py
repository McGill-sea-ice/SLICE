...
station_labels = ['DesBaillets']
station_type = 'cities'

freezeup_loc_list = ['Lasalle']
freezeup_type = 'charts'
...
for iloc,loc in enumerate(water_name_list):
    ...
    ax_tw.plot(time,T_freezeup, '*',color=plt.get_cmap('tab20')((iloc+1)*2))
...
for iloc,loc in enumerate(water_name_list):
    ...
    bp_doy = ax_fddoy[1].boxplot(freezeup_doy[17:28,iloc][mask], positions = [iloc],whis='range',showfliers=True,labels = [station_labels[iloc]])
    bp_diff = ax2.boxplot(freezeup_doy[17:28,iloc][mask]-chart_fd_doy[iloc,17:28,0][mask], positions = [iloc+0.3],whis='range',showfliers=True,labels = [station_labels[iloc]])
