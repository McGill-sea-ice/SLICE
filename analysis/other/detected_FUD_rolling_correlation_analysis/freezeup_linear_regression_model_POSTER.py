...
def freezeup_multiple_linear_regression_model(df,x_model,nyears,training_size,rolling_training = False,verbose = False):
    ...
    if not rolling_training:
        ...
        # print(xf[:,0])
        ax.plot(xf[0:-1,0],yf_hat[0:-1], 'o--', color= plt.get_cmap('tab20')(4))
        ax.plot(np.array(df['Year'][0:training_size]),yh_hat[0,:], '-', color= plt.get_cmap('tab20')(2))
        ax.set_xlabel('Year')
        ax.set_ylabel('Freeze-up DOY')

        return np.array(df['Year']),np.array(df['Freeze-up']),xf[0:-1,0],yf,yf_hat[0:-1],np.array(df['Year'][0:training_size]),yh_hat[0,:]
...
ax.plot(xall,np.ones(yall.size)*358,'-',color=[0.91,0.91,0.91])
ax.plot(xall,np.ones(yall.size)*365,'-',color=plt.get_cmap('tab20')(9))
...
# BELOW WAS FOR POSTER BUT I THINK THERE IS A PROBLEM WITH THOSE NUMBERS....
# ML_forecast = np.array([382., 358.8, 365.9, 359.22222222,
       # 370.0625, 366., 365.75, 365.0625,381.6875])
ML_forecast = np.array([356.06666667, 364.53333333, 373.53333333, 366.13333333,
       368.6       , 368.8       , 365.13333333, 361.6       ,
       371.26666667])
ax.plot(xf,ML_forecast, 'o:', color= plt.get_cmap('tab20')(0))



mae_f = np.nanmean(np.abs(np.squeeze(yf_true[:-1])-ML_forecast))
_, Rsqr_f = linear_fit(np.squeeze(yf_true[:-1]),ML_forecast)
# _, Rsqr_f = linear_fit(np.squeeze(yf_true[:-1]),np.squeeze(yf))

print(mae_f, Rsqr_f)


mae_f = np.nanmean(np.abs(np.squeeze(yf_true[:-1])-np.ones(yf.size)*365))
_, Rsqr_f = linear_fit(np.squeeze(yf_true[:-1]),np.ones(yf.size)*365)
# _, Rsqr_f = linear_fit(np.squeeze(yf_true[:-1]),np.squeeze(yf))

print(mae_f, Rsqr_f)


mae_f = np.nanmean(np.abs(np.squeeze(yall)-np.ones(yall.size)*355))
_, Rsqr_f = linear_fit(np.squeeze(yall),np.ones(yall.size)*355)
print(mae_f, Rsqr_f)
