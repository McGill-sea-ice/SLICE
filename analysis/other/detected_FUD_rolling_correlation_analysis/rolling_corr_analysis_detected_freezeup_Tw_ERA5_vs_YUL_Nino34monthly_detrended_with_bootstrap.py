...
#%%
Nino34_data = np.load(fp+'Nino34_index_NOAA/Nino34_index_NOAA_monthly.npz',allow_pickle='TRUE')
Nino34_vars = Nino34_data['Nino34_data']
Nino34_varnames = ['Nino34']

if deseasonalize:
    Nwindow = 31
    Nino34_vars = deasonalize_ts(Nwindow,Nino34_vars,Nino34_varnames,'all_time',time,years)

Nino34_vars_all = np.zeros((len(years),len(window_arr),end_dates_arr.shape[1],2))*np.nan
Nino34_max_vars_all = np.zeros((len(years),len(window_arr),end_dates_arr.shape[1],2))*np.nan
Nino34_min_vars_all = np.zeros((len(years),len(window_arr),end_dates_arr.shape[1],2))*np.nan
# boot_iyears_all = np.zeros((len(years),end_dates_arr.shape[1],nboot))*np.nan
for iend in range(end_dates_arr.shape[1]):
    Nino34_vars_all[:,:,iend,:]= get_window_monthly_vars(Nino34_vars,['Avg. monthly Nino34'],np.squeeze(end_dates_arr[:,iend]),window_arr,years,time,month_start_day)
    Nino34_max_vars_all[:,:,iend,:]= get_window_monthly_vars(Nino34_vars,['Max. monthly Nino34'],np.squeeze(end_dates_arr[:,iend]),window_arr,years,time,month_start_day)
    Nino34_min_vars_all[:,:,iend,:]= get_window_monthly_vars(Nino34_vars,['Min. monthly Nino34'],np.squeeze(end_dates_arr[:,iend]),window_arr,years,time,month_start_day)


Nino34_vars_all = np.expand_dims(np.expand_dims(Nino34_vars_all[:,:,:,:],axis=-1),axis=0)
Nino34_max_vars_all = np.expand_dims(np.expand_dims(Nino34_max_vars_all[:,:,:,:],axis=-1),axis=0)
Nino34_min_vars_all = np.expand_dims(np.expand_dims(Nino34_min_vars_all[:,:,:,:],axis=-1),axis=0)


nvars = Nino34_vars_all.shape[0]
nyears = Nino34_vars_all.shape[1]
nwindows = Nino34_vars_all.shape[2]
nend = Nino34_vars_all.shape[3]
nwindowtype = Nino34_vars_all.shape[4]
nlocs = Nino34_vars_all.shape[5]

#%%
if detrend:
    Nino34_vars_all_detrended = np.zeros(Nino34_vars_all.shape)*np.nan
    for ivar in range(nvars):
        for iw in range(nwindows):
            for iend in range(nend):
                for ip in range(nwindowtype):
                    for iloc in range(nlocs):
                        xvar = Nino34_vars_all[ivar,:,iw,iend,ip,iloc]
                        yvar = avg_freezeup_doy

                        Nino34_vars_all_detrended[ivar,:,iw,iend,ip,iloc], avg_freezeup_doy_detrended = detrend_ts(xvar,yvar,years,anomaly)
                        # Nino34_vars_all_detrended = Nino34_vars_all.copy()
else:
    Nino34_vars_all_detrended = Nino34_vars_all.copy()
...
for ivar in range(nvars):
    for iw in range(nwindows):
        for iend in range(nend):
            for ip in range(nwindowtype):
                for iloc in range(nlocs):
                    xvar = Nino34_vars_all_detrended[ivar,:,iw,iend,ip,iloc]
...
#     locnames='Montreal'
#     multilocs_rollingcorr_correlation_plot(window_arr,np.expand_dims(np.expand_dims(Nino34_vars_all[:,:,iend,:,:],axis=-1),axis=0),yvar,boot_iyears_all[:,iend,:].astype('int'),Nino34_varnames,locnames,p_critical,detrend,anomaly,enddate_str)
...
Nino34_monthly = Nino34_vars_all_detrended[0,:,:,iend,ip,0]
# Nino34_monthly = Nino34_min_vars_all[0,:,:,iend,ip,0]

# plt.figure();plt.plot(years,Nino34_monthly[:,7]);plt.title('Avg. Nino34 in April')
# plt.figure();plt.plot(years,Nino34_monthly[:,6]);plt.title('Avg. Nino34 in May')
# plt.figure();plt.plot(years,Nino34_monthly[:,5]);plt.title('Avg. Nino34 in June')
# plt.figure();plt.plot(years,Nino34_monthly[:,4]);plt.title('Avg. Nino34 in July')
# plt.figure();plt.plot(years,Nino34_monthly[:,3]);plt.title('Avg. Nino34 in August')
# plt.figure();plt.plot(years,Nino34_monthly[:,2]);plt.title('Avg. Nino34 in Sept.')
# plt.figure();plt.plot(years,Nino34_monthly[:,1]);plt.title('Avg. Nino34 in Oct.')
# plt.figure();plt.plot(years,Nino34_monthly[:,0]);plt.title('Avg. Nino34 in Nov.')
...
ax2.set_ylabel('Avg. Nino34 anomaly in Nov.', color= plt.get_cmap('tab10')(1))
...
ax2.set_ylabel('Avg. Nino34 anomaly in May', color= plt.get_cmap('tab10')(1))
...
# ax2.set_ylabel('- Nino34 Sept. + Nino34 Nov.', color= plt.get_cmap('tab10')(1))
...
# ax2=ax.twinx()
# ax2.plot(years,x,'o--', color= plt.get_cmap('tab10')(1))
# ax2.set_ylabel('Avg. Nino34 anomaly in Aug.', color= plt.get_cmap('tab10')(1))
# lincoeff, Rsqr = linear_fit(x,avg_freezeup_doy_detrended)
# r =np.sqrt(Rsqr)
# if lincoeff[0]<0: r*=-1
# ax.text(2016,21,'r = %3.2f'%(r))
...
# ax2=ax.twinx()
# ax2.plot(years,x+y,'o--', color= plt.get_cmap('tab10')(1))
# ax2.set_ylabel('Nino34 Aug. + Nino34 Sept.', color= plt.get_cmap('tab10')(1))
# lincoeff, Rsqr = linear_fit(x+y,avg_freezeup_doy_detrended)
# r =np.sqrt(Rsqr)
# if lincoeff[0]<0: r*=-1
# ax.text(2016,21,'r = %3.2f'%(r))
...
# ax.set_xlabel('Year')
# ax.set_ylabel('Nino34 Sept.', color= plt.get_cmap('tab10')(0))
# ax.grid()
# ax2=ax.twinx()
# ax2.plot(years,x,'o--', color= plt.get_cmap('tab10')(1))
# ax2.set_ylabel('Nino34 Aug.', color= plt.get_cmap('tab10')(1))
# lincoeff, Rsqr = linear_fit(x,y)
# r =np.sqrt(Rsqr)
# if lincoeff[0]<0: r*=-1
# ax.text(2016,1,'r = %3.2f'%(r))
# #%%
# Nino34_monthly = Nino34_vars_all_detrended[0,:,:,0,ip,0]
# x = Nino34_monthly[:,-1]
# fig, ax = plt.subplots()
# ax.plot(years,avg_freezeup_doy_detrended,'o-', color= plt.get_cmap('tab10')(0))
# ax.set_xlabel('Year')
...
# ax2=ax.twinx()
# ax2.plot(years,x,'o--', color= plt.get_cmap('tab10')(1))
# ax2.set_ylabel('Avg. Nino34 anomaly in April', color= plt.get_cmap('tab10')(1))
# lincoeff, Rsqr = linear_fit(x,avg_freezeup_doy_detrended)
# r =np.sqrt(Rsqr)
# if lincoeff[0]<0: r*=-1
# ax.text(2016,21,'r = %3.2f'%(r))
...
# #%%
# Nino34_monthly = Nino34_vars_all_detrended[0,:,:,0,ip,0]
# x = Nino34_monthly[:,3]+Nino34_monthly[:,2]
# y = Nino34_monthly[:,7]
# fig, ax = plt.subplots()
# ax.plot(years,avg_freezeup_doy_detrended,'o-', color= plt.get_cmap('tab10')(0))
# ax.set_xlabel('Year')
...
# ax2=ax.twinx()
# ax2.plot(years,-x+y,'o--', color= plt.get_cmap('tab10')(1))
# ax2.set_ylabel('- Nino34 Aug.- Nino34 Sept. + Nino34 April', color= plt.get_cmap('tab10')(1))
# lincoeff, Rsqr = linear_fit(-x+y,avg_freezeup_doy_detrended)
# r =np.sqrt(Rsqr)
# if lincoeff[0]<0: r*=-1
# ax.text(2016,21,'r = %3.2f'%(r))
...
# plt.xlabel('Avg. Nino34 anomaly in Sept.')
# plt.ylabel('Freezeup Anomaly')

# plt.figure()
# plt.plot(y,avg_freezeup_doy_detrended,'o')
# plt.xlabel('Avg. Nino34 anomaly in Nov.')
# plt.ylabel('Freezeup Anomaly')

# plt.figure()
# plt.plot(x,y,'o')
# plt.xlabel('Avg. Nino34 anomaly in Sept.')
# plt.ylabel('Avg. Nino34 anomaly in Nov.')
...
# plt.figure()
# plt.plot(y,avg_freezeup_doy_detrended,'o')
# plt.xlabel('Avg. Nino34 anomaly in Aug.')
# plt.ylabel('Freezeup Anomaly')

# plt.figure()
# plt.plot(x,y,'o')
# plt.xlabel('Avg. Nino34 anomaly in Sept.')
# plt.ylabel('Avg. Nino34 anomaly in Aug.')
