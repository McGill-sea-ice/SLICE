...
#%%
ONI_data = np.load(fp+'ONI_index_NOAA/ONI_index_NOAA_monthly.npz',allow_pickle='TRUE')
ONI_vars = ONI_data['ONI_data']
ONI_varnames = ['ONI']

if deseasonalize:
    Nwindow = 31
    ONI_vars = deasonalize_ts(Nwindow,ONI_vars,ONI_varnames,'all_time',time,years)

ONI_vars_all = np.zeros((len(years),len(window_arr),end_dates_arr.shape[1],2))*np.nan
ONI_max_vars_all = np.zeros((len(years),len(window_arr),end_dates_arr.shape[1],2))*np.nan
ONI_min_vars_all = np.zeros((len(years),len(window_arr),end_dates_arr.shape[1],2))*np.nan
# boot_iyears_all = np.zeros((len(years),end_dates_arr.shape[1],nboot))*np.nan
for iend in range(end_dates_arr.shape[1]):
    ONI_vars_all[:,:,iend,:]= get_window_monthly_vars(ONI_vars,['Avg. monthly ONI'],np.squeeze(end_dates_arr[:,iend]),window_arr,years,time,month_start_day)
    ONI_max_vars_all[:,:,iend,:]= get_window_monthly_vars(ONI_vars,['Max. monthly ONI'],np.squeeze(end_dates_arr[:,iend]),window_arr,years,time,month_start_day)
    ONI_min_vars_all[:,:,iend,:]= get_window_monthly_vars(ONI_vars,['Min. monthly ONI'],np.squeeze(end_dates_arr[:,iend]),window_arr,years,time,month_start_day)


ONI_vars_all = np.expand_dims(np.expand_dims(ONI_vars_all[:,:,:,:],axis=-1),axis=0)
ONI_max_vars_all = np.expand_dims(np.expand_dims(ONI_max_vars_all[:,:,:,:],axis=-1),axis=0)
ONI_min_vars_all = np.expand_dims(np.expand_dims(ONI_min_vars_all[:,:,:,:],axis=-1),axis=0)


nvars = ONI_vars_all.shape[0]
nyears = ONI_vars_all.shape[1]
nwindows = ONI_vars_all.shape[2]
nend = ONI_vars_all.shape[3]
nwindowtype = ONI_vars_all.shape[4]
nlocs = ONI_vars_all.shape[5]

#%%
if detrend:
    ONI_vars_all_detrended = np.zeros(ONI_vars_all.shape)*np.nan
    for ivar in range(nvars):
        for iw in range(nwindows):
            for iend in range(nend):
                for ip in range(nwindowtype):
                    for iloc in range(nlocs):
                        xvar = ONI_vars_all[ivar,:,iw,iend,ip,iloc]
                        yvar = avg_freezeup_doy

                        ONI_vars_all_detrended[ivar,:,iw,iend,ip,iloc], avg_freezeup_doy_detrended = detrend_ts(xvar,yvar,years,anomaly)
                        # ONI_vars_all_detrended = ONI_vars_all.copy()
else:
    ONI_vars_all_detrended = ONI_vars_all.copy()
...
for ivar in range(nvars):
    for iw in range(nwindows):
        for iend in range(nend):
            for ip in range(nwindowtype):
                for iloc in range(nlocs):
                    xvar = ONI_vars_all_detrended[ivar,:,iw,iend,ip,iloc]
...
#     locnames='Montreal'
#     multilocs_rollingcorr_correlation_plot(window_arr,np.expand_dims(np.expand_dims(ONI_vars_all[:,:,iend,:,:],axis=-1),axis=0),yvar,boot_iyears_all[:,iend,:].astype('int'),ONI_varnames,locnames,p_critical,detrend,anomaly,enddate_str)
...
ONI_monthly = ONI_vars_all_detrended[0,:,:,iend,ip,0]
# ONI_monthly = ONI_min_vars_all[0,:,:,iend,ip,0]

# plt.figure();plt.plot(years,ONI_monthly[:,7]);plt.title('Avg. ONI in April')
# plt.figure();plt.plot(years,ONI_monthly[:,6]);plt.title('Avg. ONI in May')
# plt.figure();plt.plot(years,ONI_monthly[:,5]);plt.title('Avg. ONI in June')
# plt.figure();plt.plot(years,ONI_monthly[:,4]);plt.title('Avg. ONI in July')
# plt.figure();plt.plot(years,ONI_monthly[:,3]);plt.title('Avg. ONI in August')
# plt.figure();plt.plot(years,ONI_monthly[:,2]);plt.title('Avg. ONI in Sept.')
# plt.figure();plt.plot(years,ONI_monthly[:,1]);plt.title('Avg. ONI in Oct.')
# plt.figure();plt.plot(years,ONI_monthly[:,0]);plt.title('Avg. ONI in Nov.')
...
ax2.set_ylabel('Avg. ONI anomaly in Nov.', color= plt.get_cmap('tab10')(1))
...
ax2.set_ylabel('Avg. ONI anomaly in May', color= plt.get_cmap('tab10')(1))
...
# ax2.set_ylabel('- ONI Sept. + ONI Nov.', color= plt.get_cmap('tab10')(1))
...
# ax2=ax.twinx()
# ax2.plot(years,x,'o--', color= plt.get_cmap('tab10')(1))
# ax2.set_ylabel('Avg. ONI anomaly in Aug.', color= plt.get_cmap('tab10')(1))
# lincoeff, Rsqr = linear_fit(x,avg_freezeup_doy_detrended)
# r =np.sqrt(Rsqr)
# if lincoeff[0]<0: r*=-1
# ax.text(2016,21,'r = %3.2f'%(r))
...
# ax2=ax.twinx()
# ax2.plot(years,x+y,'o--', color= plt.get_cmap('tab10')(1))
# ax2.set_ylabel('ONI Aug. + ONI Sept.', color= plt.get_cmap('tab10')(1))
# lincoeff, Rsqr = linear_fit(x+y,avg_freezeup_doy_detrended)
# r =np.sqrt(Rsqr)
# if lincoeff[0]<0: r*=-1
# ax.text(2016,21,'r = %3.2f'%(r))
...
# ax.set_xlabel('Year')
# ax.set_ylabel('ONI Sept.', color= plt.get_cmap('tab10')(0))
# ax.grid()
# ax2=ax.twinx()
# ax2.plot(years,x,'o--', color= plt.get_cmap('tab10')(1))
# ax2.set_ylabel('ONI Aug.', color= plt.get_cmap('tab10')(1))
# lincoeff, Rsqr = linear_fit(x,y)
# r =np.sqrt(Rsqr)
# if lincoeff[0]<0: r*=-1
# ax.text(2016,1,'r = %3.2f'%(r))
# #%%
# ONI_monthly = ONI_vars_all_detrended[0,:,:,0,ip,0]
# x = ONI_monthly[:,-1]
# fig, ax = plt.subplots()
# ax.plot(years,avg_freezeup_doy_detrended,'o-', color= plt.get_cmap('tab10')(0))
# ax.set_xlabel('Year')
...
# ax2=ax.twinx()
# ax2.plot(years,x,'o--', color= plt.get_cmap('tab10')(1))
# ax2.set_ylabel('Avg. ONI anomaly in April', color= plt.get_cmap('tab10')(1))
# lincoeff, Rsqr = linear_fit(x,avg_freezeup_doy_detrended)
# r =np.sqrt(Rsqr)
# if lincoeff[0]<0: r*=-1
# ax.text(2016,21,'r = %3.2f'%(r))
...
# ax2=ax.twinx()
# ax2.plot(years,-x+y,'o--', color= plt.get_cmap('tab10')(1))
# ax2.set_ylabel('- ONI Aug.- ONI Sept. + ONI April', color= plt.get_cmap('tab10')(1))
# lincoeff, Rsqr = linear_fit(-x+y,avg_freezeup_doy_detrended)
# r =np.sqrt(Rsqr)
# if lincoeff[0]<0: r*=-1
# ax.text(2016,21,'r = %3.2f'%(r))
...
# plt.xlabel('Avg. ONI anomaly in Sept.')
# plt.ylabel('Freezeup Anomaly')

# plt.figure()
# plt.plot(y,avg_freezeup_doy_detrended,'o')
# plt.xlabel('Avg. ONI anomaly in Nov.')
# plt.ylabel('Freezeup Anomaly')

# plt.figure()
# plt.plot(x,y,'o')
# plt.xlabel('Avg. ONI anomaly in Sept.')
# plt.ylabel('Avg. ONI anomaly in Nov.')
...
# plt.figure()
# plt.plot(y,avg_freezeup_doy_detrended,'o')
# plt.xlabel('Avg. ONI anomaly in Aug.')
# plt.ylabel('Freezeup Anomaly')

# plt.figure()
# plt.plot(x,y,'o')
# plt.xlabel('Avg. ONI anomaly in Sept.')
# plt.ylabel('Avg. ONI anomaly in Aug.')
