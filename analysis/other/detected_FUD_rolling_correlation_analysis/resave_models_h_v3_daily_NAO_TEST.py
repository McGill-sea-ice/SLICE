...
NAO_monthly_data = np.load(fp+'NAO_index_NOAA/NAO_index_NOAA_monthly.npz',allow_pickle='TRUE')
NAO_monthly_vars = NAO_monthly_data['data']
NAO_monthly_varnames = ['NAO']

if deseasonalize:
    NAO_monthly_vars = deseasonalize_ts(Nwindow,NAO_monthly_vars,NAO_monthly_varnames,'all_time',time,years)

NAO_monthly_vars_all = np.zeros((len(years),len(window_arr),end_dates_arr.shape[1],2))*np.nan
# boot_iyears_all = np.zeros((len(years),end_dates_arr.shape[1],nboot))*np.nan
for iend in range(end_dates_arr.shape[1]):
    NAO_monthly_vars_all[:,:,iend,:]= get_window_monthly_vars(NAO_monthly_vars,['Avg. monthly NAO'],np.squeeze(end_dates_arr[:,iend]),window_arr,years,time,month_start_day)

NAO_monthly_vars_all = np.expand_dims(np.expand_dims(NAO_monthly_vars_all[:,:,:,:],axis=-1),axis=0)
NAO_monthly_vars_all = np.squeeze(NAO_monthly_vars_all[:,:,:,:,0,:])


# LOAD DAILY NAO DATA
NAO_data = np.load(fp+'NAO_daily/NAO_daily.npz',allow_pickle='TRUE')
...
NAO_vars_all    = np.zeros((1,len(years),len(window_arr),end_dates_arr.shape[1],2,1))*np.nan
# boot_iyears_all = np.zeros((len(years),end_dates_arr.shape[1],nboot))*np.nan
for iend in range(end_dates_arr.shape[1]):
    NAO_vars_all[0,:,:,iend,:,0]= get_window_vars(NAO_vars,['Avg. daily NAO'],np.squeeze(end_dates_arr[:,iend]),window_arr,years,time,month_start_day)
NAO_vars_all = np.squeeze(NAO_vars_all[0,:,:,:,0,0])
...
NAO_monthly_vars_all = NAO_monthly_vars_all[iy_start:iy_end,:,:]
...
if detrend:
    ...
    NAO_monthly_vars_all_detrended = np.zeros(NAO_monthly_vars_all.shape)*np.nan
    ...
    for iw in range(nwindows):
         for iend in range(nend):
             xvar = NAO_monthly_vars_all[:,iw,iend]
             yvar = PDO_vars_all[:,iw,iend]
             zvar = ENSO_vars_all[:,iw,iend]
             Tvar = Twater_vars_all[:,iw,iend]
             wvar = NAO_vars_all[:,iw,iend]

             NAO_monthly_vars_all_detrended[:,iw,iend] = detrend_ts(xvar,years,anomaly)
             NAO_vars_all_detrended[:,iw,iend] = detrend_ts(wvar,years,anomaly)
    ...
    NAO_monthly_vars_all_detrended = NAO_monthly_vars_all.copy()
...
NAO_daily_monthly = np.zeros((nyears,11))
...
NAO_monthly[:,3:]= np.fliplr(NAO_monthly_vars_all_detrended[:,:,0])
NAO_monthly[:,0:3]= np.fliplr(NAO_monthly_vars_all_detrended[:,5:8,-1])

NAO_daily_monthly[:,3:]= np.fliplr(NAO_vars_all_detrended[:,:,0])
NAO_daily_monthly[:,0:3]= np.fliplr(NAO_vars_all_detrended[:,5:8,-1])
...
Apr_NAO_daily = NAO_daily_monthly[:,3]
...
Nov_NAO_daily = NAO_daily_monthly[:,10]
...
data = {'Year': years,
        ...
        'Apr. NAO (daily) Anomaly': Apr_NAO_daily,
        ...
        'Aug. NAO (daily) Anomaly': Aug_NAO_daily,

        'Sept. RH Anomaly': Sept_RH,
        'Sept. Cloud Anomaly': Sept_clouds,
        'Sept. NAO (daily) Anomaly': Sept_NAO_daily,
        ...
        'Nov. NAO (daily) Anomaly': Nov_NAO_daily,
...
columns=['Year',
        ...
        'May Ta_avg Anomaly',
        'May Twater Anomaly',
        'Aug. NAO (daily) Anomaly',
        'Sept. RH Anomaly',
        'Sept. Cloud Anomaly',
        'Sept. NAO (daily) Anomaly',
        'Sept. SLP Anomaly',
        'Sept. precip Anomaly',
...
# npred = 2
# stats_h_Dec1_2pred_all15, stats_f_Dec1_2pred_all15, stats_h_Dec1_2pred_s15, stats_f_Dec1_2pred_s15  = MLR_model_analysis(fpath,npred,start_date,training_size,xall,yall,show=show)
# pred2_list15 = stats_f_Dec1_2pred_s15[['Pred. 1','Pred. 2']]

# npred = 3
# stats_h_Dec1_3pred_all15, stats_f_Dec1_3pred_all15, stats_h_Dec1_3pred_s15, stats_f_Dec1_3pred_s15  = MLR_model_analysis(fpath,npred,start_date,training_size,xall,yall,show=show)
# pred3_list15 = stats_f_Dec1_3pred_s15[['Pred. 1','Pred. 2','Pred. 3']]

# npred = 4
# stats_h_Dec1_4pred_all15, stats_f_Dec1_4pred_all15, stats_h_Dec1_4pred_s15, stats_f_Dec1_4pred_s15 = MLR_model_analysis(fpath,npred,start_date,training_size,xall,yall,show=show)
# pred4_list15 = stats_f_Dec1_4pred_s15[['Pred. 1','Pred. 2','Pred. 3','Pred. 4']]


# print(stats_f_Dec1_2pred_s15.shape[0],stats_f_Dec1_3pred_s15.shape[0],stats_f_Dec1_4pred_s15.shape[0])



#%%
# def resave_models(fpath,npred,start_date,training_size,xall,yall,show=True,anomaly=False):
...
# model_data = np.load(fpath+'models'+str(npred)+'_it434404_ts'+str(training_size)+'.npz',allow_pickle=True)
...
for im in range(x_models.shape[0]):
    x_model = [x_models[im][m] for m in range(npred)]

    [models_h,xall,yall,xf,yf_true,yf,xh_plot,xh,yh,
    mae_h,rmse_h,Rsqr_h,Rsqr_adj_h,std_h,
    mae_f,rmse_f,Rsqr_f,Rsqr_adj_f, std_f]  = freezeup_multiple_linear_regression_model(df[['Year','Freeze-up','Freeze-up Anomaly']+x_model],x_model,nyears,training_size,anomaly = anomaly,rolling_training=rolling_training,verbose=False)
...
for im in range(len(x_models_2)):
    ...
    for h in range(yh_models_2[0].shape[0]):
        try:
            r_tmp[h] = predicted_r2(np.squeeze(yh_true_models_2[im][h,:]),np.squeeze(yh_models_2[im][h,:]),np.squeeze(xh_models_2[im][h,:]))
...
x_models_2_Nov1 = []
mae_f_models_2_Nov1 = []
rmse_f_models_2_Nov1 = []
Rsqr_f_models_2_Nov1 = []
Rsqr_adj_f_models_2_Nov1 = []
std_f_models_2_Nov1 = []
mae_h_models_2_Nov1 = []
rmse_h_models_2_Nov1 = []
Rsqr_h_models_2_Nov1 = []
Rsqr_adj_h_models_2_Nov1 = []
Rsqr_pred_h_models_2_Nov1 = []
std_h_models_2_Nov1 = []
xf_models_2_Nov1 = []
yf_models_2_Nov1 = []
xh_plot_models_2_Nov1 = []
xh_models_2_Nov1 = []
yh_models_2_Nov1 = []
yall_models_2_Nov1 = []
xall_models_2_Nov1 = []
imodels_2_Nov1 = []
yh_true_models_2_Nov1 = []

for i in range(len(x_models_2)):
    xim = [x_models_2[i][m] for m in range(npred)]
    if (~np.any(['Apr. aFDD' in xim[j] for j in range(npred)]) ) & (~np.any(['Nov' in xim[j] for j in range(npred)])) & (~np.any(['Fall' in xim[j] for j in range(npred)])):
        ...
        if coeff[0] > 0 :
            imodels_2_Nov1 += [i]
            models_2_Nov1 += [models_2[i]]
            x_models_2_Nov1 += [x_models_2[i]]
            mae_f_models_2_Nov1 += [mae_f_models_2[i]]
            rmse_f_models_2_Nov1 += [rmse_f_models_2[i]]
            Rsqr_f_models_2_Nov1 += [Rsqr_f_models_2[i]]
            Rsqr_adj_f_models_2_Nov1 += [Rsqr_adj_f_models_2[i]]
            std_f_models_2_Nov1 += [std_f_models_2[i]]
            mae_h_models_2_Nov1 += [mae_h_models_2[i]]
            rmse_h_models_2_Nov1 += [rmse_h_models_2[i]]
            Rsqr_h_models_2_Nov1 += [Rsqr_h_models_2[i]]
            Rsqr_adj_h_models_2_Nov1 += [Rsqr_adj_h_models_2[i]]
            Rsqr_pred_h_models_2_Nov1 += [Rsqr_pred_h_models_2[i]]
            std_h_models_2_Nov1 += [std_h_models_2[i]]
            xf_models_2_Nov1 += [xf_models_2[i]]
            yf_models_2_Nov1 += [yf_models_2[i]]
            xh_plot_models_2_Nov1 += [xh_plot_models_2[i]]
            xh_models_2_Nov1 += [xh_models_2[i]]
            yh_models_2_Nov1 += [yh_models_2[i]]
            yall_models_2_Nov1 += [yall_models_2[i]]
            xall_models_2_Nov1 += [xall_models_2[i]]
            yh_true_models_2_Nov1 += [yh_true_models_2[i]]


models_2_Dec1 = []
x_models_2_Dec1 = []
mae_f_models_2_Dec1 = []
rmse_f_models_2_Dec1 = []
Rsqr_f_models_2_Dec1 = []
Rsqr_adj_f_models_2_Dec1 = []
std_f_models_2_Dec1 = []
mae_h_models_2_Dec1 = []
rmse_h_models_2_Dec1 = []
Rsqr_h_models_2_Dec1 = []
Rsqr_adj_h_models_2_Dec1 = []
Rsqr_pred_h_models_2_Dec1 = []
std_h_models_2_Dec1 = []
xf_models_2_Dec1 = []
yf_models_2_Dec1 = []
xh_plot_models_2_Dec1 = []
xh_models_2_Dec1 = []
yh_models_2_Dec1 = []
yall_models_2_Dec1 = []
xall_models_2_Dec1 = []
imodels_2_Dec1 = []
yh_true_models_2_Dec1 = []
...
for i in range(len(x_models_2)):
    ...
    if (~np.any(['Apr. aFDD' in xim[j] for j in range(npred)]) ) :
        ...
        if coeff[0] > 0 :
            imodels_2_Dec1 += [i]
            models_2_Dec1 += [models_2[i]]
            x_models_2_Dec1 += [x_models_2[i]]
            mae_f_models_2_Dec1 += [mae_f_models_2[i]]
            rmse_f_models_2_Dec1 += [rmse_f_models_2[i]]
            Rsqr_f_models_2_Dec1 += [Rsqr_f_models_2[i]]
            Rsqr_adj_f_models_2_Dec1 += [Rsqr_adj_f_models_2[i]]
            std_f_models_2_Dec1 += [std_f_models_2[i]]
            mae_h_models_2_Dec1 += [mae_h_models_2[i]]
            rmse_h_models_2_Dec1 += [rmse_h_models_2[i]]
            Rsqr_h_models_2_Dec1 += [Rsqr_h_models_2[i]]
            Rsqr_adj_h_models_2_Dec1 += [Rsqr_adj_h_models_2[i]]
            Rsqr_pred_h_models_2_Dec1 += [Rsqr_pred_h_models_2[i]]
            std_h_models_2_Dec1 += [std_h_models_2[i]]
            xf_models_2_Dec1 += [xf_models_2[i]]
            yf_models_2_Dec1 += [yf_models_2[i]]
            xh_plot_models_2_Dec1 += [xh_plot_models_2[i]]
            xh_models_2_Dec1 += [xh_models_2[i]]
            yh_models_2_Dec1 += [yh_models_2[i]]
            yall_models_2_Dec1 += [yall_models_2[i]]
            xall_models_2_Dec1 += [xall_models_2[i]]
            yh_true_models_2_Dec1 += [yh_true_models_2[i]]


np.savez(fpath+'models'+str(npred)+'_ts'+str(training_size)+'_Nov1'+'TEST',
            models_h = models_2_Nov1,
            x_models = x_models_2_Nov1,
            mae_f_models = mae_f_models_2_Nov1,
            rmse_f_models = rmse_f_models_2_Nov1,
            Rsqr_f_models = Rsqr_f_models_2_Nov1,
            Rsqr_adj_f_models = Rsqr_adj_f_models_2_Nov1,
            std_f_models = std_f_models_2_Nov1,
            mae_h_models = mae_h_models_2_Nov1,
            rmse_h_models = rmse_h_models_2_Nov1,
            Rsqr_h_models =  Rsqr_h_models_2_Nov1,
            Rsqr_adj_h_models = Rsqr_adj_h_models_2_Nov1,
            Rsqr_pred_h_models = Rsqr_pred_h_models_2_Nov1,
            std_h_models = std_h_models_2_Nov1,
            xf_models = xf_models_2_Nov1,
            yf_models = yf_models_2_Nov1,
            xh_models = xh_models_2_Nov1,
            xh_plot_models = xh_plot_models_2_Nov1,
            yh_models = yh_models_2_Nov1,
            yh_true_models = yh_true_models_2_Nov1,
            yall = yall_models_2_Nov1,
            xall = xall_models_2_Nov1,
            anomaly = anomaly,
...
np.savez(fpath+'models'+str(npred)+'_ts'+str(training_size)+'_Dec1'+'TEST',
            models_h = models_2_Dec1,
            x_models = x_models_2_Dec1,
            mae_f_models = mae_f_models_2_Dec1,
            rmse_f_models = rmse_f_models_2_Dec1,
            Rsqr_f_models = Rsqr_f_models_2_Dec1,
            Rsqr_adj_f_models = Rsqr_adj_f_models_2_Dec1,
            std_f_models = std_f_models_2_Dec1,
            mae_h_models = mae_h_models_2_Dec1,
            rmse_h_models = rmse_h_models_2_Dec1,
            Rsqr_h_models =  Rsqr_h_models_2_Dec1,
            Rsqr_adj_h_models = Rsqr_adj_h_models_2_Dec1,
            Rsqr_pred_h_models = Rsqr_pred_h_models_2_Dec1,
            std_h_models = std_h_models_2_Dec1,
            xf_models = xf_models_2_Dec1,
            yf_models = yf_models_2_Dec1,
            xh_models = xh_models_2_Dec1,
            xh_plot_models = xh_plot_models_2_Dec1,
            yh_models = yh_models_2_Dec1,
            yh_true_models = yh_true_models_2_Dec1,
            yall = yall_models_2_Dec1,
            xall = xall_models_2_Dec1,
            anomaly = anomaly,
            deseasonalize = deseasonalize ,
            detrend = detrend,
            station_labels = station_labels,
...
np.savez(fpath+'models'+str(npred)+'_ts'+str(training_size)+'TEST',
            models_h = models_2,
            x_models = x_models_2,
            mae_f_models = mae_f_models_2,
            rmse_f_models = rmse_f_models_2,
            Rsqr_f_models = Rsqr_f_models_2,
            Rsqr_adj_f_models = Rsqr_adj_f_models_2,
            std_f_models = std_f_models_2,
            mae_h_models = mae_h_models_2,
            rmse_h_models = rmse_h_models_2,
            Rsqr_h_models =  Rsqr_h_models_2,
            Rsqr_adj_h_models = Rsqr_adj_h_models_2,
            Rsqr_pred_h_models = Rsqr_pred_h_models_2,
            std_h_models = std_h_models_2,
            xf_models = xf_models_2,
            yf_models = yf_models_2,
            xh_models = xh_models_2,
            xh_plot_models = xh_plot_models_2,
            yh_models = yh_models_2,
            yh_true_models = yh_true_models_2,
            yall = yall_models_2,
            xall = xall_models_2,
            anomaly = anomaly,
            deseasonalize = deseasonalize ,
