...
# Now get December (lead = 0) & January (lead =1) Forecast
cansips_frcst_tmp = get_monthly_forecast(feature,region,month,years_cansips)
cansips_frcst[:,0:2] = cansips_frcst_tmp[:,0:2] -273.15


# NOW ADD DJF FORECASTS:
# Get Forecast Ensemble Mean starting Dec.
startmonth = 12
ensm_avg_monthly,_,_,_,_ = ensemble_clim_monthly(feature,region,startmonth,year_start=ys,year_end=ye)
# Get DJF forecast climatology by averaging December (lead = 0) & January (lead =1)
# & February (lead = 2) during training period
ensm_avg_DJF = np.nanmean(ensm_avg_monthly[:,0:3],axis=1)
# Get average, std, and percentile of NDJ forecasts over the training period
cansips_clim[2] = np.nanmean(ensm_avg_DJF-273.15)
cansips_clim_std[2] = np.nanstd(ensm_avg_DJF-273.15)
cansips_clim_p33[2] = np.nanpercentile(ensm_avg_DJF-273.15,(1/3.)*100)
cansips_clim_p66[2] = np.nanpercentile(ensm_avg_DJF-273.15,(2/3.)*100)
...
ax[1].set_ylabel('Dec. T$_a$ ($^{\circ}$C)\n(lead = 0 month)')
ax[1].grid(linestyle=':')
...
# ax[2].set_ylabel('Jan. T$_a$ ($^{\circ}$C)\n(lead = 1 month)')
# ax[2].grid(linestyle=':')

im = 2
ax[2].fill_betweenx(np.arange(-5,-1,(4)/10.),np.ones(10)*ys,np.ones(10)*ye,
                color=color_train , alpha=0.1)
ax[2].plot(years_cansips,np.ones(len(years_cansips))*cansips_clim[im],'--',color = 'gray',linewidth=1)
ax[2].fill_between(years_cansips,np.ones(len(years_cansips))*(cansips_clim_p33[im]),np.ones(len(years_cansips))*(cansips_clim_p66[im]),color='gray',alpha=0.2)
ax[2].plot(years_cansips,cansips_frcst[:,im],'-o',color='k')
...
ax[2].plot(years_all,plus,'o',color=color_plus)
ax[2].plot(years_all,minus,'o',color=color_minus)
ax[2].set_ylabel('DJF T$_a$ ($^{\circ}$C)\n(lead = 0-2 month)')
ax[2].grid(linestyle=':')
...
# detrend_ts(var_in,years,anomaly_type)
