...
#         np.savez(p_dir+'seasonal_corr'+str(p_critical)+'_CanSIPS_'+feature_list[feat]+'_ensm_'+ 'anom'*anomaly+ 'fcst'*(~anomaly) +'_'+'LongueuilFUD'+'_'+season_str[season]+'_'+str(years_cansips[iyr_start])+'_'+str(years_cansips[iyr_end]),
#                   rsqr=rsqr,
#                   corr=corr,
#                   signif=signif,
#                   data_corr = data_corr)
...
for iseason in range(4):
    ...
    plt.suptitle(season_str[iseason]+' Forecast of ' + feature_list[feat] + ' vs FUD Longueuil')

    for lead in range(iseason+1):
        data_tmp = np.load(p_dir+'seasonal_corr'+str(p_critical)+'_CanSIPS_'+feature_list[feat]+'_ensm_'+ 'anom'*anomaly+ 'fcst'*(~anomaly) +'_'+'LongueuilFUD'+'_'+season_str[iseason]+'_'+str(years_cansips[iyr_start])+'_'+str(years_cansips[iyr_end])+'.npz')
        rsqr=data_tmp['rsqr']
