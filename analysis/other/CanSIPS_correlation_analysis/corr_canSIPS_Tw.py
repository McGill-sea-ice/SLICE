...
#         np.savez(p_dir+'corr'+str(p_critical)+'_CanSIPS_'+feature_list[feat]+'_ensm_'+ 'anom'*anomaly+ 'fcst'*(~anomaly) +'_'+'LongueuilTw'+'_lead_'+str(lead)+'_'+str(years_cansips[iyr_start])+'_'+str(years_cansips[iyr_end]),
#                   rsqr=rsqr,
#                   corr=corr,
#                   signif=signif,
                  # data_corr = data_corr)
...
#     data_tmp = np.load(p_dir+'corr'+str(p_critical)+'_CanSIPS_'+feature_list[feat]+'_ensm_'+ 'anom'*anomaly+ 'fcst'*(~anomaly) +'_'+'LongueuilTw'+'_lead_'+str(lead)+'_'+str(years_cansips[iyr_start])+'_'+str(years_cansips[iyr_end])+'.npz')
#     rsqr=data_tmp['rsqr']
...
plt.suptitle('Forecast of ' + feature_list[feat] + ' in Dec. vs avg Dec. Tw Longueuil')
...
fig,ax = plt.subplots(nrows=2,ncols=2,figsize=(18,8),sharex=True,sharey=True,subplot_kw={'projection': ccrs.PlateCarree()})
plt.suptitle('Forecast of ' + feature_list[feat] + ' in Nov. vs avg Dec. Tw Longueuil')
...
fig,ax = plt.subplots(nrows=2,ncols=2,figsize=(18,8),sharex=True,sharey=True,subplot_kw={'projection': ccrs.PlateCarree()})
plt.suptitle('Forecast of ' + feature_list[feat] + ' in Oct. vs avg Dec. Tw Longueuil')
...
# FORECAST IN SEP VS FUD
fig,ax = plt.subplots(nrows=2,ncols=2,figsize=(18,8),sharex=True,sharey=True,subplot_kw={'projection': ccrs.PlateCarree()})
plt.suptitle('Forecast of ' + feature_list[feat] + ' in Sep. vs avg Dec. Tw Longueuil')
...
feat = 2
lead = 0
data_tmp = np.load(p_dir+'corr'+str(p_critical)+'_CanSIPS_'+feature_list[feat]+'_ensm_'+ 'anom' +'_'+'LongueuilTw'+'_lead_'+str(lead)+'_'+str(years_cansips[iyr_start])+'_'+str(years_cansips[iyr_end])+'.npz')
rsqr=data_tmp['rsqr']
signif=data_tmp['signif']
corr=data_tmp['corr']
data_corr = data_tmp['data_corr']
...
if region == 'test':
    rlon1, rlat1 = 360-80.5, 46.5
    rlon2, rlat2 = 360-75.5, 49.5
...
Tw_det, [slope,b] = detrend_ts(np.squeeze(monthly_avg_Twater[:,11]),years,anomaly_type)

# NOTE: monthly_avg_Twater was already detrended earlier
y = np.squeeze(monthly_avg_Twater[:,11])
...
ax.plot(years,avg_freezeup_doy_det,'*-',color='gray')
ax.grid()
ax.set_xlabel('Year')
ax.set_ylabel('Avg Dec. Tw Anomaly (deg. C)',color='black')
...
fig,ax=plt.subplots()
title_str = 'Detrended Forecast Anom. for '+title_month+ ' '+feature_list[feat]+' (lead = '+str(lead)+' months)'

xplot = ts
xlabel = 'Detrended Forecast Anomaly'
yplot = avg_freezeup_doy_det
ylabel = 'FUD Anomaly (days)'
...
ax.plot(np.ones(len(np.arange(np.nanmin(yplot),np.nanmax(yplot))))*np.nanmean(xplot),np.arange(np.nanmin(yplot),np.nanmax(yplot)),'-',color='k')
ax.plot(np.ones(len(np.arange(np.nanmin(yplot),np.nanmax(yplot))))*(np.nanmean(xplot)+0.55*np.nanstd(xplot)),np.arange(np.nanmin(yplot),np.nanmax(yplot)),':',color='k')
ax.plot(np.ones(len(np.arange(np.nanmin(yplot),np.nanmax(yplot))))*(np.nanmean(xplot)-0.55*np.nanstd(xplot)),np.arange(np.nanmin(yplot),np.nanmax(yplot)),':',color='k')

ax.plot(np.arange(np.nanmin(xplot),np.nanmax(xplot),0.1),np.ones(len(np.arange(np.nanmin(xplot),np.nanmax(xplot),0.1)))*np.nanmean(yplot),'-',color=plt.get_cmap('tab20')(0))
ax.plot(np.arange(np.nanmin(xplot),np.nanmax(xplot),0.1),np.ones(len(np.arange(np.nanmin(xplot),np.nanmax(xplot),0.1)))*(np.nanmean(yplot)+0.55*np.nanstd(yplot)),':',color=plt.get_cmap('tab20')(0))
ax.plot(np.arange(np.nanmin(xplot),np.nanmax(xplot),0.1),np.ones(len(np.arange(np.nanmin(xplot),np.nanmax(xplot),0.1)))*(np.nanmean(yplot)-0.55*np.nanstd(yplot)),':',color=plt.get_cmap('tab20')(0))
