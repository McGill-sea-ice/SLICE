...
from functions_encoderdecoder import execute_fold_tpf_allmembers_continuoustraining
from functions_encoderdecoder import execute_fold_tpf_allmembers_continuoustraining_nonrecursive
...
# suffix = '_reducelrexp_0_05_lr0_004_RWE_redo'
...
# suffix = '_reducelrexp_0_05_lr0_004_dropout_0_5_RWE_allmembers_continuoustraining_withsnow'
# suffix = '_lr0_016_PFE_allmembers_continuoustraining_withsnow'
# suffix = '_reducelrexp_0_02_lr0_0001_RWE_'+str(nb_members)+'members_continuoustraining_withsnow'
# suffix = '_reducelrexp_0_02_lr0_0001_RWE_allmembers_continuoustraining_withsnow'
suffix = '_reducelrexp_0_05_lr0_004_RWE_ensemblemean_nonrecursive_withsnow_trainshuffled'
suffix = '_reducelrexp_0_02_lr0_0001_minlr_1e5_PFE_nonrecursive_withsnow_trainshuffled'
suffix = '_reducelrexp_0_02_lr0_0004_minlr_1e5_PFE_nonrecursive_withsnow_trainshuffled'
...
with tf.device(device_name):
    ...
    *   Daily time series of monthly and seasonal forecasts from CanSIPS (*see below*)
    ...
    if perfect_forecast:
        ...
        # n_f_vars = (not ensemble_mean_fcst)*len(forecast_vars)*nb_members+ (ensemble_mean_fcst)*len(forecast_vars)
        n_f_vars = len(forecast_vars)
    ...
    if valid_scheme == 'LOOk':
        ...
        # for iyr_test,yr_test in enumerate(test_years[10:12]):
            ...
            if (not ensemble_mean_fcst) & (not perfect_forecast):
            ...
            if nfolds > 0:
                for ifold,[train_index, valid_index] in enumerate(kf.split(ind_kfold)):
                    ...
                    _,y_pred_valid,_,
                    _,y_valid,_,
                    _,y_clim_valid,_] = execute_fold_tpf_allmembers_continuoustraining_nonrecursive(df_t_train_fold, df_t_valid_fold, df_t_test_fold,
            ...
            y_train,_,y_test,
            y_clim_train,_,y_clim_test] = execute_fold_tpf_allmembers_continuoustraining_nonrecursive(df_t_train, df_t_valid, df_t_test,
            ...
            if save_model_outputs:
                ...
                np.savez(save_path+'/output/encoderdecoder_horizon'+str(pred_len)+'_context'+str(input_len)+'_nneurons'+str(latent_dim)+'_seed'+str(seed)+'_nepochs'+str(n_epochs)+(nb_layers > 1)*('_'+str(nb_layers)+'layers')+'_'+norm_type+'_'+dense_act_func_name+'_'+loss_name+anomaly_target*'_anomaly_target'+suffix+'_'+str(yr_test),
    ...
    if valid_scheme == 'standard':
        ...
        y_clim_train,y_clim_valid,y_clim_test] = execute_fold_tpf_allmembers_continuoustraining_nonrecursive(df_t_train, df_t_valid, df_t_test,
                                                                                                          df_p_train, df_p_valid, df_p_test,
                                                                                                          df_f_train, df_f_valid, df_f_test,
        ...
        if save_model_outputs:
            ...
            np.savez(save_path+'/output/encoderdecoder_horizon'+str(pred_len)+'_context'+str(input_len)+'_nneurons'+str(latent_dim)+'_seed'+str(seed)+'_nepochs'+str(n_epochs)+(nb_layers > 1)*('_'+str(nb_layers)+'layers')+'_'+norm_type+'_'+dense_act_func_name+'_'+loss_name+anomaly_target*'_anomaly_target'+suffix,
