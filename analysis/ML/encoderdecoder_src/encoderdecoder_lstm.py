...
from functions_encoderdecoder import fit_scaler, normalize_df, replace_nan_with_clim, encoder_decoder_recursive
from functions_encoderdecoder import create_dataset_tpf, reconstruct_ysamples_tpf, create_dataset_tpf
from functions_encoderdecoder import SEAS5_dailyarray_to_ts, obs_dailyts_to_forecast_ts
from functions_encoderdecoder import execute_fold
...
train_yr_start = 1993 # Training dataset: 1993 - 2010
valid_yr_start = 2020 # Validation dataset: 2011 - 2015
test_yr_start = 2020 # Testing dataset: 2016 - 2021
...
ensemble_mean_train = True
# ensemble_mean_train = False
...
nb_members = 25
nb_members = np.min([nb_members, 25]) # Max nb of members = 25.
...
# save_path = '/mnt/crunch/amelie/slice/prog/analysis/ML/encoderdecoder_src/'
save_path = '/storage/amelie/slice/prog/analysis/ML/encoderdecoder_src/'

suffix = '_reducelrexp_0_05_lr0_004'+'_PFE'*perfect_forecast + ('_RWE'+ '_allmembers'*(not ensemble_mean_fcst)+ '_ensemblemean'*(ensemble_mean_fcst))*(not perfect_forecast) + '_nonrecursive'*nonrecursive +'_withsnow'+ ('_continuoustraining'*continuous_training)*(not ensemble_mean_train)+ '_trainshuffled'*shuffle_training  +'_EMtrain'*ensemble_mean_train
# suffix = suffix+'_TEST_SAVE_MODEL'
...
model_name = 'encoderdecoder_horizon'+str(pred_len)+'_context'+str(input_len)+'_nneurons'+str(latent_dim)+'_seed'+str(seed)+'_nepochs'+str(n_epochs)+(nb_layers > 1)*('_'+str(nb_layers)+'layers')+'_'+norm_type+'_'+dense_act_func_name+'_'+loss_name+anomaly_target*'_anomaly_target'+suffix+'_nonrecursive'*nonrecursive + '_trainshuffled'*shuffle_training

model_save_name = save_path+'models/'+model_name
save_name = save_path+'output/'+model_name
...
with tf.device(device_name):
    ...
    *   Daily time series of monthly and seasonal forecasts from CanSIPS (*see below*)
    ...
    filepath = 'slice/data/daily_predictors/'
    ...
    if perfect_forecast:
        ...
        # First run the following command to save 'SEAS5_SEAS51_MLR_forecast_daily_predictors.npz'
        # Then it can be loaded in future runs because it takes a while to save otherwise.
        fdir_SEAS5 = local_path+'/slice/data/processed/SEAS5/'
        fname_SEAS5 = 'SEAS5_SEAS51_forecast_daily_predictors.npz'
        # [_,_,_,_] = save_SEAS5_predictor_arrays(local_path,time_rep='daily',region='D')
        ...
        df_f = np.zeros((len(time),len(forecast_vars)*12*nb_members))*np.nan
        df_f_col = []
        for iv,fv in enumerate(forecast_vars):
            if fv == 'Avg. Ta_mean':
                sv = 'SEAS5_Ta_mean'
            ...
            SEAS5_f = data_SEAS5[sv][:,:,:,:nb_members]
            for im in range(SEAS5_f.shape[3]):
                for month in range(1,13):
                    ...
                    df_f[:,12*(im+iv*nb_members)+month-1] = SEAS5_ts[it0_SEAS5:it1_SEAS5+1]

        if (not ensemble_mean_train) & (not continuous_training):
            n_f_vars = len(forecast_vars)*25
        else:
            n_f_vars = len(forecast_vars)
    ...
    if valid_scheme == 'LOOk':
        ...
        for iyr_test,yr_test in enumerate(test_years):
            ...
            if (not perfect_forecast) & (not ensemble_mean_fcst):
                y_valid_all = np.zeros((nfolds,365*28*10,pred_len))*np.nan
                y_pred_valid_all = np.zeros((nfolds,365*28*10,pred_len))*np.nan
                y_clim_valid_all = np.zeros((nfolds,365*28*10,pred_len))*np.nan
                target_time_valid_all = np.zeros((nfolds,365*28*10,pred_len))*np.nan
            else:
            ...
            if nfolds > 0:
                for ifold,[train_index, valid_index] in enumerate(kf.split(ind_kfold)):
                    ...
                    _,y_pred_valid,_,
                    _,y_valid,_,
                    _,y_clim_valid,_,_,_,_,_,_,_,_,_,_,_,_] = execute_fold(df_t_train_fold, df_t_valid_fold, df_t_test_fold,
            ...
            y_train,_,y_test,
            y_clim_train,_,y_clim_test,
            train_windowed,valid_windowed,test_windowed,
            train_clim_windowed,valid_clim_windowed,test_clim_windowed,
            time_train_windowed,time_valid_windowed,time_test_windowed,
            scaler_t,scaled_FU_threshold] = execute_fold(df_t_train, df_t_valid, df_t_test,
            ...
            if (save_model_outputs):
                if (not os.path.exists(save_name+'_'+str(yr_test)+'.npz')) | (os.path.exists(save_name+'_'+str(yr_test)+'.npz') & overwrite_outputs):
                    print('SAVING OUTPUTS...')
                    np.savez(save_name+'_'+str(yr_test),
    ...
    if valid_scheme == 'standard':
        ...
        y_clim_train,y_clim_valid,y_clim_test,
        train_windowed,valid_windowed,test_windowed,
        train_clim_windowed,valid_clim_windowed,test_clim_windowed,
        time_train_windowed,time_valid_windowed,time_test_windowed,
        scaler_t,scaled_FU_threshold] = execute_fold(df_t_train, df_t_valid, df_t_test,
                                                                ...
                                                                ensemble_mean_fcst,
                                                                nb_members,
                                                                model_save_name+'_train_yrs_'+str(train_years[0])+'_'+str(train_years[-1]),
                                                                save_model = save_model,
                                                                nonrecursive = nonrecursive,
                                                                continuous_training = continuous_training,
                                                                shuffle_training = shuffle_training,
        ...
        if (save_model_outputs):
            if (not os.path.exists(save_name+'.npz')) | (os.path.exists(save_name+'.npz') & overwrite_outputs):
                print('SAVING OUTPUTS...')
                np.savez(save_name,
                    target_time_train=target_time_train,
...
# #%%
# plot_sample(y_pred,y,y_clim,target_time,it=85,pred_type=pred_type,show_clim=True)
# # plot_sample(y_pred,y,y_clim,target_time,it=490,pred_type=pred_type,show_clim=True)
# # plot_sample(y_pred,y,y_clim,target_time,it=491,pred_type=pred_type,show_clim=True)
# # plot_sample(y_pred,y,y_clim,target_time,it=491+25,pred_type=pred_type,show_clim=True)
