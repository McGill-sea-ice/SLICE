...
istart_label = ['Oct 1st', 'Oct 15th', 'Nov 1st', 'Nov 15th', 'Dec. 1st', 'Dec.15th']
month_istart = [10,10,11,11,12,12]
day_istart = [1,15,1,15,1,15]
...
MAE_arr = np.zeros((6,len(nb_layers_list),len(input_window_list)))*np.nan
RMSE_arr = np.zeros((6,len(nb_layers_list),len(input_window_list)))*np.nan
Rsqr_arr = np.zeros((6,len(nb_layers_list),len(input_window_list)))*np.nan
Rsqradj_arr = np.zeros((6,len(nb_layers_list),len(input_window_list)))*np.nan
Acc_arr = np.zeros((6,len(nb_layers_list),len(input_window_list)))*np.nan
# MAE_clim_arr = np.zeros((6,len(nb_layers_list),len(input_window_list)))*np.nan
# RMSE_clim_arr = np.zeros((6,len(nb_layers_list),len(input_window_list)))*np.nan
# Rsqr_clim_arr = np.zeros((6,len(nb_layers_list),len(input_window_list)))*np.nan
# Acc_clim_arr = np.zeros((6,len(nb_layers_list),len(input_window_list)))*np.nan
...
if freezeup_opt == 1:
    Longueuil_FUD = np.array([ np.nan, 360., 358., 364., 342., 365., 350., 365., 360., 343., 367.,
            ...
            347., 365., 371., 351., 348., 356., 354.,  np.nan,  np.nan])
...
for il,nlayers in enumerate(nb_layers_list):
    for iw,inpw in enumerate(input_window_list):
        ...
        for i_split in range(nsplits):
            ...
            if use_softplus:
                if model_name == 'MLP':
                    pred_data = np.load('./'+model_name+'_'+ valid_type +'valid/softplus/'+model_name+'_valid'+ str(start_yr+i_split) +'_horizon'+str(lblw)+'_context'+str(inpw)+'_nlayers'+str(nlayers)+'_nneurons'+str(nneurons)+'_nepochs'+str(ne)+'_'+norm_type+'_'+suffix+'.npz',allow_pickle='TRUE')
                if model_name == 'LSTM':
                    pred_data = np.load('./'+model_name+'_'+ valid_type +'valid/softplus/'+model_name+'_valid'+ str(start_yr+i_split) +'_horizon'+str(lblw)+'_context'+str(inpw)+'_nlayers'+str(nlayers)+'_hiddensize'+str(hs)+'_nepochs'+str(ne)+'_'+norm_type+'_'+suffix+'.npz',allow_pickle='TRUE')
            else:
                if model_name == 'MLP':
                    pred_data = np.load('./'+model_name+'_'+ valid_type +'valid/'+model_name+'_valid'+ str(start_yr+i_split) +'_horizon'+str(lblw)+'_context'+str(inpw)+'_nlayers'+str(nlayers)+'_nneurons'+str(nneurons)+'_nepochs'+str(ne)+'_'+norm_type+'_'+suffix+'.npz',allow_pickle='TRUE')
                if model_name == 'LSTM':
                    pred_data = np.load('./'+model_name+'_'+ valid_type +'valid/'+model_name+'_valid'+ str(start_yr+i_split) +'_horizon'+str(lblw)+'_context'+str(inpw)+'_nlayers'+str(nlayers)+'_hiddensize'+str(hs)+'_nepochs'+str(ne)+'_'+norm_type+'_'+suffix+'.npz',allow_pickle='TRUE')
            ...
            for istart in range(0,MAE_arr.shape[0],2):
                if istart == 0:
                    month = 10
                if istart == 2:
                    month = 11
                    ...
                    for s in range(samples.shape[0]):
                        ...
                        if year >= years_all_splits[0]:
                            ...
                            if (np.sum(mask_freeze_sample) > 0):
                                if fd_sample[0] == year:
                                    if calendar.isleap(years_all_splits[iyr]):
                                        freezeup_dates_sample[istart+1,i_split,0] = iyr
                                        freezeup_dates_sample[istart+1,i_split,1:4] = fd_sample
                                        freezeup_dates_sample_doy[istart+1,i_split]= (dt.date(int(fd_sample[0]),int(fd_sample[1]),int(fd_sample[2]))-dt.date(int(fd_sample[0]),1,1)).days
                                    else:
                                        freezeup_dates_sample[istart+1,i_split,0] = iyr
                                        freezeup_dates_sample[istart+1,i_split,1:4] = fd_sample
                                        freezeup_dates_sample_doy[istart+1,i_split]= (dt.date(int(fd_sample[0]),int(fd_sample[1]),int(fd_sample[2]))-dt.date(int(fd_sample[0]),1,1)).days+1
                                else:
                                    if calendar.isleap(years_all_splits[iyr]):
                                        freezeup_dates_sample[istart+1,i_split,0] = iyr
                                        freezeup_dates_sample[istart+1,i_split,1:4] = fd_sample
                                        freezeup_dates_sample_doy[istart+1,i_split]= fd_sample[2]+365
                                    else:
                                        freezeup_dates_sample[istart+1,i_split,0] = iyr
                                        freezeup_dates_sample[istart+1,i_split,1:4] = fd_sample
                                        freezeup_dates_sample_doy[istart+1,i_split]= fd_sample[2]+365
                            ...
                            if (np.sum(mask_freeze_target) > 0):
                                if fd_target[0] == year:
                                    if calendar.isleap(years_all_splits[iyr]):
                                        freezeup_dates_target[istart+1,i_split,0] = iyr
                                        freezeup_dates_target[istart+1,i_split,1:4] = fd_target
                                        freezeup_dates_target_doy[istart+1,i_split]= (dt.date(int(fd_target[0]),int(fd_target[1]),int(fd_target[2]))-dt.date(int(fd_target[0]),1,1)).days
                                    else:
                                        freezeup_dates_target[istart+1,i_split,0] = iyr
                                        freezeup_dates_target[istart+1,i_split,1:4] = fd_target
                                        freezeup_dates_target_doy[istart+1,i_split]= (dt.date(int(fd_target[0]),int(fd_target[1]),int(fd_target[2]))-dt.date(int(fd_target[0]),1,1)).days+1
                                else:
                                    if calendar.isleap(years_all_splits[iyr]):
                                        freezeup_dates_target[istart+1,i_split,0] = iyr
                                        freezeup_dates_target[istart+1,i_split,1:4] = fd_target
                                        freezeup_dates_target_doy[istart+1,i_split]= fd_target[2]+365
                                    else:
                                        freezeup_dates_target[istart+1,i_split,0] = iyr
                                        freezeup_dates_target[istart+1,i_split,1:4] = fd_target
                                        freezeup_dates_target_doy[istart+1,i_split]= fd_target[2]+365
                            ...
                            if (np.sum(mask_freeze_clim_target) > 0):
                                if fd_clim_target[0] == year:
                                    freezeup_dates_clim_target[istart+1,i_split,0] = iyr
                                    if calendar.isleap(years_all_splits[iyr]):
                                        freezeup_dates_clim_target[istart+1,i_split,1] = fd_clim_target[0]
                                        freezeup_dates_clim_target[istart+1,i_split,2] = fd_clim_target[1]
                                        freezeup_dates_clim_target[istart+1,i_split,3] = fd_clim_target[2]+1
                                        freezeup_dates_clim_target_doy[istart+1,i_split]= (dt.date(int(fd_clim_target[0]),int(fd_clim_target[1]),int(fd_clim_target[2]))-dt.date(int(fd_clim_target[0]),1,1)).days+1
                                    else:
                                        freezeup_dates_clim_target[istart+1,i_split,1:4] = fd_clim_target
                                        freezeup_dates_clim_target_doy[istart+1,i_split]= (dt.date(int(fd_clim_target[0]),int(fd_clim_target[1]),int(fd_clim_target[2]))-dt.date(int(fd_clim_target[0]),1,1)).days+1
        ...
        for istart in range(freezeup_dates_sample.shape[0]):
            ...
            if np.all(np.isnan(fu_mae[istart,:])):
                MAE_arr[istart,il,iw] = np.nan
            else:
                MAE_arr[istart,il,iw] = np.nansum(fu_mae[istart,:])/n

            if np.all(np.isnan(fu_rmse[istart,:])):
                RMSE_arr[istart,il,iw] = np.nan
            else:
                RMSE_arr[istart,il,iw] = np.sqrt(np.nansum(fu_rmse[istart,:])/n)

            if  np.all(np.isnan(fu_acc[istart,:])):
                Acc_arr[istart,il,iw] = np.nan
            else:
                Acc_arr[istart,il,iw] = np.nansum(fu_acc[istart,:])/n

            if (np.all(np.isnan(ts_doy))) | (np.all(np.isnan(Longueuil_FUD_period))) :
                Rsqr_arr[istart,il,iw] = np.nan
                Rsqradj_arr[istart,il,iw] = np.nan
            else:
                model = sm.OLS(Longueuil_FUD_period, sm.add_constant(ts_doy,has_constant='skip'), missing='drop').fit()
                Rsqr_arr[istart,il,iw] = model.rsquared
                Rsqradj_arr[istart,il,iw] = model.rsquared_adj
        ...
        if plot_FUD_ts:
            ...
            for ic,istart in enumerate([2,3,4]):
                fd_ML_forcast = np.zeros((len(years_all_splits)))*np.nan
                for iyr in range(len(years_all_splits)):
                ...
                plt.plot(years_all_splits,fd_ML_forcast,'o:', color=plt.get_cmap('tab20c')(7-ic), label= 'MLP - '+istart_label[istart])
                if ~np.all(np.isnan(fd_ML_forcast)):
                    ...
                    print('START DATE: ' + istart_label[istart])
                    print('Rsqr: ',model.rsquared, model.rsquared_adj)
                    print('MAE: ', MAE_arr[istart,il,iw])
                    print('RMSE: ',RMSE_arr[istart,il,iw])
                    print('Acc.: ',Acc_arr[istart,il,iw])
...
for i,istart in enumerate(istart_plot):
    p_mae=ax_mae[i].imshow(MAE_arr[istart],vmin=3,vmax=9)
...
# fig_rmse,ax_rmse = plt.subplots(nrows = 1, ncols = len(istart_plot),figsize=(14,3))
# letter = ['a)','b)','c)']
# for i,istart in enumerate(istart_plot):
#     p_rmse=ax_rmse[i].imshow(RMSE_arr[istart],vmin=4,vmax=10)
#     ax_rmse[0].set_ylabel('Nb. Layers')
#     #ax_rmse[1].set_xlabel('Context Window (days)')
#     ax_rmse[i].set_yticks(np.arange(len(nb_layers_list)))
#     ax_rmse[i].set_yticklabels([str(nb_layers_list[k]) for k in range(len(nb_layers_list))])
#     ax_rmse[i].set_xticks(np.arange(len(input_window_list)))
#     ax_rmse[i].set_xticklabels([str(input_window_list[k]) for k in range(len(input_window_list))])
#     ax_rmse[i].set_title(letter[i]+' Forecast start: '+istart_label[istart])

# fig_rmse.subplots_adjust(left=0.2)
# cbar_rmse_ax = fig_rmse.add_axes([0.07, 0.18, 0.02, 0.66])
# fig_rmse.colorbar(p_rmse, cax=cbar_rmse_ax)
# cbar_rmse_ax.text(-0.95,5.6,'RMSE (days)',fontsize=14, rotation = 90)

fig_acc,ax_acc = plt.subplots(nrows = 1, ncols = len(istart_plot),figsize=(14,3))
letter = ['d)','e)','f)']
for i,istart in enumerate(istart_plot):
    p_acc=ax_acc[i].imshow(Acc_arr[istart]*100,vmin=10,vmax=80)
...
letter = ['g)','h)','i)']
for i,istart in enumerate(istart_plot):
    p_Rsqr=ax_Rsqr[i].imshow(Rsqr_arr[istart],vmin=0.1,vmax=0.90)
