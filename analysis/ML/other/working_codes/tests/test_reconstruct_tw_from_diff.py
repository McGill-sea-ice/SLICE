...
label_col = [np.where(df.columns == target_label)[0][0]]
input_col = np.where(df.columns != target_label)[0].tolist()
if include_target_as_pred:
    input_col.append(label_col[0])
    input_col.sort()
# clim_col = [-1]

# yr_input_train, yr_target_train, _ , _ = sliding_window_samples(df_train['Avg. Twater'].values,time_train,input_width,label_width,shift,nslide,input_col,label_col)
...
yr_input_train = input_train[:,:,np.where(df_train.columns == 'Avg. Twater')[0][0]]
yr_input_valid = input_valid[:,:,np.where(df_valid.columns == 'Avg. Twater')[0][0]]
yr_input_test = input_test[:,:,np.where(df_test.columns == 'Avg. Twater')[0][0]]
...
class DenseMulti(nn.Module):
    global normalize_target,norm_type
...
time_in = time_valid
Tw_climatology_mean = Tw_climatology_mean_valid
t_range = train_range_tar
t_offset = train_offset_tar
...
#     if ytransform == 'diff':
#         if normalize_predictors:
#             yr = (yr_input*train_range_pred[np.where(df_valid.columns == 'Avg. Twater')[0][0]]) + train_offset_pred[np.where(df_valid.columns == 'Avg. Twater')[0][0]]
#         else:
#             yr = yr_input
