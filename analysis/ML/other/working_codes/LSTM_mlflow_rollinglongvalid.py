...
mlflow_track = False
mlflow_path = 'file://'+base_path+'/slice/prog/mlflow/mlruns'
...
# vars_out = ['Twater','Ta_mean','FDD']
# vars_out = ['Twater','Ta_mean','FDD','snowfall']
# vars_out = ['Twater','Ta_mean','FDD','snowfall','SLP']
...
for hidden in hidden_list:
    for input_window in input_window_list:
        ...
        nsplits = 2020-valid_yr_start+1

        for i_split in range(nsplits):
            it_train_start = np.where(time == (dt.date(train_yr_start,3,1)-date_ref).days)[0][0]
            it_train_end = np.where(time == (dt.date(valid_yr_start+i_split,2,28)-date_ref).days)[0][0]

            it_valid_start = np.where(time == (dt.date(valid_yr_start+i_split,3,1)-date_ref).days)[0][0]
            it_valid_end = np.where(time == (dt.date(valid_yr_start+i_split+1,2,28)-date_ref).days)[0][0]
            ...
            class LSTMLinear(nn.Module):
                ...
                def __init__(self, input_dim, output_dim, seqlen_target, hidden_size, n_layers):
                    ...
                    self.fc = nn.Linear(hidden_size, seqlen_target) #fully connected last layer
                ...
                def forward(self, x):
                    ...
                    # (note: the weights are initialized to zero by default, but I leave this here so that there is a guide for changing the weights if necessary in the future)
                    h_0 = torch.zeros(self.num_layers, x.size(0), self.hidden_size,device=device, dtype=torch.float32)
                    c_0 = torch.zeros(self.num_layers, x.size(0), self.hidden_size,device=device, dtype=torch.float32)
            ...
            np.savez(base_path+'/slice/data/ML_output/'+'/LSTM_'+valid_scheme+'valid/'+use_softplus*'softplus/'+model_name+'_valid'+ str(valid_yr_start+i_split) +'_horizon'+str(label_width)+'_context'+str(input_width)+'_nlayers'+str(nb_layers)+'_hiddensize'+str(hidden)+'_nepochs'+str(n_epochs)+'_'+norm_type+'_'+suffix,
