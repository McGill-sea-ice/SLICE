...
#%%
# nb_layers_list = [3,6,2,4]
# input_window_list = [240,120,30,60,90]
nb_layers_list = [3,6,2]
input_window_list = [240,120]
...
for nb_layers in nb_layers_list:
    for input_window in input_window_list:
        ...
        valid_yr_start = 2017 # [2008 - 2013] = 6 years
        ...
        for i_split in range(nsplits):
            it_train_start = np.where(time == (dt.date(train_yr_start+i_split,4,1)-date_ref).days)[0][0]
            it_train_end = np.where(time == (dt.date(valid_yr_start+i_split,3,31)-date_ref).days)[0][0]

            it_valid_start = np.where(time == (dt.date(valid_yr_start+i_split,4,1)-date_ref).days)[0][0]
            it_valid_end = np.where(time == (dt.date(valid_yr_start+i_split+1,3,31)-date_ref).days)[0][0]
