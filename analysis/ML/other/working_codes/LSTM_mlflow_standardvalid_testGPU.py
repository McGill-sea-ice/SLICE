...
for hidden in hidden_list:
    for input_window in input_window_list:
        ...
        for it in range(input_train.shape[0]):
            ...
            predictions_train.append(model(torch.from_numpy(input_pred).float().to(device)).detach())

        predictions_valid = []
        for it in range(input_valid.shape[0]):
            input_pred = np.expand_dims(input_valid[it,:,:],0)
            predictions_valid.append(model(torch.from_numpy(input_pred).float().to(device)).detach())

        predictions_test = []
        for it in range(input_test.shape[0]):
            input_pred = np.expand_dims(input_test[it,:,:],0)
            predictions_test.append(model(torch.from_numpy(input_pred).float().to(device)).detach())
        ...
        def evaluate_ML_pred(targets,predictions,yr_input,time_in,Tw_climatology_mean,t_range,t_offset,plot_pred_ts = True):
            ...
            MSE=nn.MSELoss()
            if normalize_target:
                targets = (np.squeeze(torch.from_numpy(targets).float().to(device))*t_range) + t_offset
                predictions = (np.squeeze(predictions)*t_range) + t_offset
            ...
            print('VALID')
            print(targets_recons.shape, targets_recons.size())
        ...
        evaluate_ML_pred(targets,predictions,yr_input_valid,time_target_valid,clim_target_valid,train_range_tar,train_offset_tar,plot_pred_ts = True)
