...
optimizer = optim.SGD(LinearModel.parameters(),lr=1e-5, momentum=0.9)
...
for it in range(input_valid.shape[0]):
    predictions.append(LinearModel(torch.from_numpy(input_valid[it,:]).float()).item())
plt.figure()
...
for it in range(input_train.shape[0]):
    ...
    predictions.append(LinearModel(torch.from_numpy(input_pred).float()).item())
