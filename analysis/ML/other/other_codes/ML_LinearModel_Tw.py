...
series=Twater
plt.figure();plt.plot(time,series)

# Split series in train-Valid sets
# (Here it is assumed that the test set will be avilable in the future as new observations
...
class NeuralNet(nn.Module):
    def __init__(self, input_size, output_size):
        ...
        self.linear_mid = nn.Linear(100, 40)
...
dl_train,input_train,target_train = windowed_dataset(y_train,y_train,seqlen_input,seqlen_target,nslide,batch_size,shuffle_train)
dl_valid,input_valid,target_valid = windowed_dataset(y_valid,y_valid,seqlen_input,seqlen_target,nslide,batch_size,shuffle_valid)
...
plt.figure()
plot_series(time_valid[seqlen_input:],target_valid)
plot_series(time_valid[seqlen_input:],predictions)
...
plot_series(time_train[seqlen_input:],np.squeeze(np.array(predictions)))
