...
vars_out = ['Twater','Ta_mean', 'NAO','precip','cloud','FDD','TDD']
...
# for pred_window in np.arange(5,10,5):
for pred_window in np.arange(64,65):
    ...
    hs = 32
    model = LSTMLinear(len(input_col),len(label_col), seqlen_target=label_width, hidden_size=hs, n_layers=1)
    model.to(device)
    model_name = 'LSTM'
    ...
    np.savez('./ML_pred_'+model_name+'_horizon_'+str(label_width)+'_input_'+str(input_width)+'_nepochs_'+str(epoch)+'hidden_'+str(hs),
