...
# plt.figure()
# plot_series_1step(time_valid[input_width:],target_valid.ravel(),'-',ax_labels=['Time', 'T$_{w}$'],linecolor='black')
# plot_series_1step(time_valid[input_width:],np.array(predictions).ravel(),'-',ax_labels=['Time', 'T$_{w}$'],linecolor=plt.get_cmap('tab20')(2))
...
#%%
# SIMPLEST TRAINANBLE ONE-STEP AHEAD:
# LINEAR (DENSE)
...
# Instantiate Baseline model
model = Linear(len(input_col),len(label_col))
model.to(device)
...
# Plot weights of the linear layer to see what
# variables are mainly used in the regression
plt.figure()
plt.bar(x = range(len(input_col)),
        height=np.squeeze(model.linear.weight.data.numpy()))
axis = plt.gca()
axis.set_xticks(range(len(input_col)))
_ = axis.set_xticklabels(colabels, rotation=45)
...
#%%
# SAME AS ABOVE (i.e. LINEAR MODEL), BUT TAKING
# MORE INPUT AT A TIME
...
# Windowing infos:
input_width,label_width,shift,nslide = 24, 24, 1, 1
input_col = [0,1,2,3,4]
label_col = [0]
...
for s in np.arange(0,target_valid.shape[0]-(input_width+shift),label_width):
    ...
    plot_series(time_valid[input_width+(nslide*s):input_width+label_width+(nslide*s)],np.array(y_renorm)[s,:],'-',ax_labels=['Time', 'T$_{w}$'],linecolor=plt.get_cmap('tab20')(s))
plt.title('Predictors: T$_{air}$, T$_{water}$, |T$_{water}$-T$_{air}$| (previous ' + '%2i'%input_width + 'days)')
...
# for s in np.arange(0,target_valid.shape[0]-(input_width+shift),label_width):
#     plot_series(time_valid[input_width+(nslide*s):input_width+label_width+(nslide*s)],np.squeeze(target_valid[s,:]),'-',ax_labels=['Time', 'T$_{w}$'],linecolor='black')
#     plot_series(time_valid[input_width+(nslide*s):input_width+label_width+(nslide*s)],np.squeeze(np.array(predictions[s])),'-',ax_labels=['Time', 'T$_{w}$'],linecolor=plt.get_cmap('tab20')(s))
# plt.title('Predictors: T$_{air}$, T$_{water}$, |T$_{water}$-T$_{air}$| (previous ' + '%2i'%input_width + 'days)')
# print(MAE(np.squeeze(torch.from_numpy(target_valid).float()),np.squeeze(torch.from_numpy(np.array(predictions).astype(float)).float())))
# print(MSE(np.squeeze(torch.from_numpy(target_valid).float()),np.squeeze(torch.from_numpy(np.array(predictions).astype(float)).float())))
# print(np.corrcoef(target_valid.ravel(),np.array(predictions).ravel())[0,1])
...
#%%
# STILL LINEAR MODEL BUT USING MULTIPLE INPUT TIME STEPS TO
...
# # variables are mainly used in the regression
# plt.figure()
# plt.bar(x = range(len(input_col)*input_width),
#         height=np.squeeze(model.linear.weight.data.numpy()))
# axis = plt.gca()
# axis.set_xticks(range(len(input_col)))
# _ = axis.set_xticklabels(colabels, rotation=45)
...
#%%
# NOW ADDING ACTIVATION FUNCTION AND DEEPER LAYERS:
...
# Instantiate Baseline model
model = Dense(len(input_col),len(label_col))
model.to(device)
...
# plt.figure()
# plt.bar(x = range(len(input_col)),
#         height=np.squeeze(model.linear_in.weight.data.numpy()))
# axis = plt.gca()
# axis.set_xticks(range(len(input_col)))
# _ = axis.set_xticklabels(colabels, rotation=45)

#%%
# NOW USE MULTIPLE INPUT STEPS TO PREDICT ONE-STEP AHEAD
# STILL USING THE DENSE MODEL
...
# MAE=nn.L1Loss()
# MSE=nn.MSELoss()
# print(MAE(np.squeeze(torch.from_numpy(target_valid).float()),np.squeeze(torch.from_numpy(np.array(predictions).astype(float)).float())).numpy()*train_std + train_mean)
# print(MSE(np.squeeze(torch.from_numpy(target_valid).float()),np.squeeze(torch.from_numpy(np.array(predictions).astype(float)).float())).numpy())
# print(np.corrcoef(target_valid.ravel(),np.array(predictions).ravel())[0,1])
# plt.figure()
# plot_series_1step(time_valid[input_width:],target_valid.ravel(),'-',ax_labels=['Time', 'T$_{w}$'],linecolor='black')
# plot_series_1step(time_valid[input_width:],np.array(predictions).ravel(),'-',ax_labels=['Time', 'T$_{w}$'],linecolor=plt.get_cmap('tab20')(2))
...
# MAE=nn.L1Loss()
# MSE=nn.MSELoss()
# print(MAE(np.squeeze(torch.from_numpy(target_test).float()),np.squeeze(torch.from_numpy(np.array(predictions).astype(float)).float())).numpy()*train_std + train_mean)
# print(MSE(np.squeeze(torch.from_numpy(target_test).float()),np.squeeze(torch.from_numpy(np.array(predictions).astype(float)).float())).numpy())
# print(np.corrcoef(target_test.ravel(),np.array(predictions).ravel())[0,1])
# plt.figure()
# plot_series_1step(time_test[input_width:],target_test.ravel(),'-',ax_labels=['Time', 'T$_{w}$'],linecolor='black')
# plot_series_1step(time_test[input_width:],np.array(predictions).ravel(),'-',ax_labels=['Time', 'T$_{w}$'],linecolor=plt.get_cmap('tab20')(2))
# plt.title('Predictors: T$_{air}$, T$_{water}$, |T$_{water}$-T$_{air}$| (previous ' + '%2i'%input_width + 'days)')
...
# plt.figure()
# plot_series_1step(time_train[input_width:],target_train.ravel(),'-',ax_labels=['Time', 'T$_{w}$'],linecolor='black')
# plot_series_1step(time_train[input_width:],np.array(predictions_train).ravel(),'-',ax_labels=['Time', 'T$_{w}$'],linecolor=plt.get_cmap('tab20')(2))
# plt.title('Predictors: T$_{air}$, T$_{water}$, |T$_{water}$-T$_{air}$| (previous ' + '%2i'%input_width + 'days)')
...
#%%
# NOW GET FANCY WITH AN LSTM MODEL TO PREDICT ONE-STEP AHEAD ONLY
...
#%%
# NOW USE MULTIPLE INPUT STEPS TO PREDICT MULTI-STEP AHEAD
# STILL USING THE DENSE MODEL
...
# Windowing infos:
input_width,label_width,shift,nslide = 24, 24, 24, 1
input_col = [0,1,2,3,4]
label_col = [0]
...
for s in np.arange(0,target_valid.shape[0]-(input_width+shift),label_width):
    plot_series(time_valid[input_width+(nslide*s):input_width+label_width+(nslide*s)],np.squeeze(target_valid[s,:]),'-',ax_labels=['Time', 'T$_{w}$'],linecolor='black')
    plot_series(time_valid[input_width+(nslide*s):input_width+label_width+(nslide*s)],np.squeeze(np.array(predictions[s])),'-',ax_labels=['Time', 'T$_{w}$'],linecolor=plt.get_cmap('tab20')(2))
plt.title('Predictors: T$_{air}$, T$_{water}$, |T$_{water}$-T$_{air}$| (previous ' + '%2i'%input_width + 'days)')
...
print(np.corrcoef(target_test.ravel(),np.array(predictions).ravel())[0,1])
plt.figure()
# for s in range(target_test.shape[0]):
for s in np.arange(0,target_test.shape[0]-(input_width+shift),label_width):
    plot_series(time_test[input_width+(nslide*s):input_width+label_width+(nslide*s)],np.squeeze(target_test[s,:]),'-',ax_labels=['Time', 'T$_{w}$'],linecolor='black')
    plot_series(time_test[input_width+(nslide*s):input_width+label_width+(nslide*s)],np.squeeze(np.array(predictions[s])),'-',ax_labels=['Time', 'T$_{w}$'],linecolor=plt.get_cmap('tab20')(2))
plt.title('Predictors: T$_{air}$, T$_{water}$, |T$_{water}$-T$_{air}$| (previous ' + '%2i'%input_width + 'days)')
