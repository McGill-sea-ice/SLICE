...
LSTMModel = LSTMLinear(input_dim=len(predictors_train), output_dim=1, seqlen_target=seqlen_target, hidden_size=8, n_layers=1)
LSTMModel.to(device)
#%%
class LSTMLinear(nn.Module):
    ...
    def forward(self, x):
        ...
        pred = F.relu(self.linear(output))
        pred = pred.transpose(0,2)
        pred = self.linout(pred)
        pred = pred.transpose(0,2)
...
class LSTMLinear(nn.Module):
    ...
    def forward(self, x):
        # the input to an LSTM must be of size (seq_len, batch_size, input_dim)
        # pred = self.flatten(x)
        # pred = self.lintest(pred)
        # pred = torch.unsqueeze(pred, 1)
        # x = x.transpose(0, 1)
        # output, _ = self.LSTM(x) # output size is: [seq_len, batch_size, hidden_size]
        # pred = self.linear(output[-self.fwindow:])
        # pred = pred.transpose(0,1)
        # # pred = output[-self.fwindow:]
...
LSTMModel = LSTMLinear(input_dim=len(predictors_train), output_dim=1, seqlen_target=seqlen_target, hidden_size=7, n_layers=1)
LSTMModel.to(device)
...
for batch_idx, (data, target) in enumerate(dl_train):
        ...
        if batch_idx == 0:
        # transfer the data on the chosen device
            data0, target0 = data.to(device), target.to(device)
            # forward propagation on the data
            example= LSTMModel(data0)
            print(batch_idx, data0.shape, target0.shape)
            print(example.shape)
...
# optimizer = optim.SGD(LSTMModel.parameters(),lr=1e-5, momentum=0.9)
