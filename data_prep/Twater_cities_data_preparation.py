...
fp = local_path+'slice/data/raw/Twater_cities/'
...
# station_list = ['Candiac']
# years = [2003,2004,2005,2006,2007,2008,2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020,2021]

# station_list = ['Atwater']
# years = [1992,1993,1994,1995,1996,1997,1998,1999,2000,2001,2002,2003,2004,2005,2006,2007,2008,2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020,2021]

station_list = ['Longueuil']
...
for l,loc_name in enumerate(station_list):
    ...
    for year in years:
        ...
        # csv_tmp = read_csv(fp+loc_name+'/Tw_'+loc_name+'_preclean.csv',skip=1)
        csv_tmp = read_csv(fp+loc_name+'/Tw_'+loc_name+'_updated_preclean.csv',skip=1)
        ...
        for month in range(12):
            for day in range(day_arr[month]):
                ...
                if len(day_csv) == 0:
                    ...
                    Tw = np.nanmean(day_csv[:,3].copy())

                Twater[indx,0] = date
                Twater[indx,1] = Tw

    # ==========================================================================
    np.savez(local_path+'slice/data/processed/Twater_cities/Twater_cities_'+loc_name+'_updated',
