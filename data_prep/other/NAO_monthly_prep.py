...
file_path = local_path+'slice/data/raw/NOAA_climate_indices/NOAA_1991_2020_monthly_NAO_index.csv'
NAO_monthly = read_csv(file_path,skip=1)
...
for it in range(len(time)):
    ...
    if date_it.year in years:
        if (date_it.year == 2021):
            if date_it.month < 6:
                NAO_year = NAO_monthly[np.where(NAO_monthly[:,0] == date_it.year)]
                NAO_month = NAO_year[np.where(NAO_year[:,1] == date_it.month)][0]
                NAO[it,0] = NAO_month[2]
        else:
            NAO_year = NAO_monthly[np.where(NAO_monthly[:,0] == date_it.year)]
            NAO_month = NAO_year[np.where(NAO_year[:,1] == date_it.month)][0]
            NAO[it,0] = NAO_month[2]

save_path = local_path+'slice/data/processed/NAO_index_NOAA/'
savename = 'NAO_index_NOAA_monthly'
np.savez(save_path+savename,data=NAO)
